...
`factory_get_rules` MCP tool before any prototype work. This bundled copy is a
...
- Update task state when you actually start/finish work; signal completion
  with `factory_signal_done` - the final decision belongs to a human.
- Sync files after every meaningful change with `factory_sync_files`.
- Call `factory_working` while actively collaborating on project work.
