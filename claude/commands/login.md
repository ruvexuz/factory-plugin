---
description: Log in to Ruvex Factory (device flow)
allowed-tools: Bash
---
...
- Windows:
  `powershell -ExecutionPolicy Bypass -File "${CLAUDE_PLUGIN_ROOT}/scripts/login.ps1"`
- macOS / Linux (and anything else):
  `sh "${CLAUDE_PLUGIN_ROOT}/scripts/login.sh"`
